...
async function getZoomAccessToken() {
    ...
    if (!accountId || !clientId || !clientSecret) {
        throw new Error('Missing Zoom OAuth credentials. Please set ZOOM_ACCOUNT_ID, ZOOM_CLIENT_ID, and ZOOM_CLIENT_SECRET environment variables.');
    }
...
async function getRecordingDownloadToken(recordingId) {
    ...
    try {
        const response = await axios_1.default.get(`https://api.zoom.us/v2/phone/recording/${recordingId}/download_access_token`, {
            headers: {
        ...
        throw new Error(`Failed to get download access token: ${error.response?.data?.message || error.message}`);
    }
}
/**
