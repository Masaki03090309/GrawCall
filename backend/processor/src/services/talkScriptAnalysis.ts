...
export async function analyzeTalkScriptMatch(
  ...
  try {
    ...
    const talkScript: TalkScript = {
      ...
      hearing_items:
        talkScriptData.talk_script_hearing_items?.map((item: any) => ({
    ...
    console.log(
      `Talk script found: ${talkScript.hearing_items.length} hearing items, opening=${!!talkScript.opening_script}, proposal=${!!talkScript.proposal_script}, closing=${!!talkScript.closing_script}`
    )
...

{
  ...
  "hearing_item_coverage": {
    ...
    // Validate and return
    if (
      typeof analysisData.overall_match_rate === 'number' &&
      analysisData.phase_match_rates &&
      analysisData.hearing_item_coverage
    ) {
      console.log(
        `Analysis complete: overall=${analysisData.overall_match_rate}%, hearing=${analysisData.phase_match_rates.hearing}%`
      )
      return {
        should_analyze: true,
 ...
 */
function buildAnalysisPrompt(talkScript: TalkScript, transcript: string): string {
 ...
 */
export function determineCausalPattern(phaseMatchRates: {
  opening: number
  hearing: number
  proposal: number
  closing: number
