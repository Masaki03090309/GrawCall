...
│  - 文字起こし（Whisper API）              │
│  - 通話状態判定（GPT-5-mini）             │
│  - トークスクリプト一致率分析（GPT-5-mini）│
│  - フィードバック生成（GPT-5-mini + RAG）  │
│  - RAG検索結果の活用（GPT-5-mini）        │
│  - 感情分析（音声周波数解析）              │
...
- **RAG活用モデル**: **GPT-5-mini**（学習資料を参照したフィードバック生成）
...
2. **GPT-5-miniによるテキスト抽出とフェーズ自動判定**
   - まずGPT-5-mini Vision APIでPDFをテキスト化
...
- GPT-5-miniで文字認識できない場合は、OCR（Tesseract等）にフォールバック
